"""
offer_agent — FastAPI route, mirrors the old `hr-dev-offer-agent` Cloud
Function (offer_agent). Runs ADK LlmAgents for a conversational offer-letter
builder (chat), a generate-and-send step (generate), a custom-template
converter (process_template), and an offers list (get_offers).

Behavioral fix vs. the original: the entry point here is `async def`, so
`_handle_chat`/`_handle_generate` now `await _run_agent_turn(...)` directly
instead of calling `asyncio.run(...)` — calling asyncio.run() from inside an
already-running event loop (which FastAPI provides) would raise a
RuntimeError. Nothing else about the agent logic changed.
"""
...
project_id    = PROJECT_ID  # local aliases, matches original variable names used in f-strings below
    ...
    try:
        ...
        if "multipart/form-data" in content_type:
            ...
            if action == "process_template":
                return await _handle_process_template(form)
            return JSONResponse({"error": f"Unknown multipart action: {action}"}, status_code=400)
        ...
        if action == "chat":
            ...
            return _handle_get_offers(body)
        else:
...
async def _handle_process_template(form):
    ...
    if 'pdf' in filename.lower() and not mime_type:
        mime_type = 'application/pdf'
    elif 'docx' in filename.lower() and not mime_type:
...
async def _handle_chat(body: dict):
    ...
    result = await _run_agent_turn(agent, session_id, user_message)
...
async def _handle_generate(body: dict):
    ...
    result = await _run_agent_turn(agent, session_id, prompt)
...
def _handle_get_offers(body: dict):
    job_id = body.get('jobId', '')
    if not job_id:
