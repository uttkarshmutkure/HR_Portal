...
def handle_candidate_data(request):
    ...
    try:
        ...
        # ── GET_FULL_PROFILE ───────────────────────────────────────────────────
            ...
            return (json.dumps({'timeline': timeline, 'feedback': feedback}), 200, headers)
        ...
        # Replaces ShortlistStore (localStorage).
            ...
            return (json.dumps({'success': True, 'candidates': candidates}), 200, headers)
        ...
        # ── UPDATE_JOB_STATUS ──────────────────────────────────────────────────
            ...
            if not job_id or not new_status:
                return (json.dumps({'error': 'Missing jobId or status'}), 400, headers)
        ...
        # ── GET_RESUME_KEY_NOTES ───────────────────────────────────────────────
            ...
            if not raw_text.strip():
                return (json.dumps({'error': 'No raw resume text stored for this candidate'}), 404, headers)
