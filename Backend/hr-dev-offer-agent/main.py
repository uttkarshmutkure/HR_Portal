...
def save_offer_to_db(
    ...
    try:
        ...
        delete_q = f"""
            DELETE FROM `{table}`
            WHERE job_id = '{job_id}' AND candidate_id = '{candidate_id}' AND status = 'draft'
        """
        ...
        if errors:
            return f"BQ insert error: {errors}"
...
def _build_chat_agent(cand_ctx: dict, job_ctx: dict, prior_offers: list, current_draft: dict, custom_fields: list = None) -> LlmAgent:
...
DEFAULT CALCULATION RULES (Annual):
1. Basic: 50% of Total CTC
2. HRA: 50% of Basic
3. PF: 12% of Basic
4. Medical: 15000 (1250 monthly)
5. Conveyance: 19200 (1600 monthly)
6. LTA: 12000 (1000 monthly)
7. Bonus: 0 (unless specified)
...
def _build_generate_agent(cand_ctx: dict, job_ctx: dict, draft: dict, send_email: bool) -> LlmAgent:
    designation          = draft.get('designation', job_ctx.get('title', ''))
    deadline             = (datetime.now(timezone.utc) + timedelta(days=7)).strftime('%-d %B %Y')
...
TOOL CALL ORDER:
1. save_offer_to_db — pass all offer fields + the full wrapped HTML as offer_html
...
def _handle_chat(body: dict, headers: dict):
    ...
    session_id = f"chat_{job_id}_{candidate_id}"
    
    agent = _build_chat_agent(cand_ctx, job_ctx, prior_offers, current_draft, custom_fields)
...
def _handle_generate(body: dict, headers: dict):
    ...
    agent      = _build_generate_agent(cand_ctx, job_ctx, draft, send_email)
    prompt     = (
