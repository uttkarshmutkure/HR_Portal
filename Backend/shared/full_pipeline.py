...
HIRING_COMPANY = config.HIRING_COMPANY

GMAIL_SENDER       = config.GMAIL_SENDER
GMAIL_APP_PASSWORD = config.GMAIL_APP_PASSWORD
RECRUITER_EMAIL    = config.RECRUITER_EMAIL

MAX_WORKERS_CANDIDATES = config.MAX_WORKERS_CANDIDATES
MAX_WORKERS_QUESTIONS  = config.MAX_WORKERS_QUESTIONS
...
Q1 — Skill Based      : First missing skill / weakest area vs JD.
Q2 — Skill Based      : Second missing / different weak area. Must differ from Q1.
...
Return ONLY this JSON. No explanation. No markdown:
[
  {{"type": "Skill Based",    "question": "question text"}},
  {{"type": "Skill Based",    "question": "question text"}},
...
def run_pipeline(job_id: str, top_k: int = 30, top_n: int = 5) -> dict:
    ...
    top_candidates    = (passed + human_review)[:top_n]   # top N for email
    all_for_questions = passed + human_review              # ALL passed + human_review get questions
    ...
    # Step 6 — parallel question generation for ALL passed + human_review
    t0 = time.time()
    log.info("Generating questions for %d candidates (%d workers)…", len(all_for_questions), MAX_WORKERS_QUESTIONS)
    with ThreadPoolExecutor(max_workers=MAX_WORKERS_QUESTIONS) as executor:
        all_for_questions = list(executor.map(
            lambda c: generate_questions_for_candidate(c, jd),
            all_for_questions,
        ))
    log.info("Step 6 done in %.1fs", time.time() - t0)

    # Sync questions back into passed/human_review and top_candidates
    questions_map = {c["candidate_id"]: c.get("questions", []) for c in all_for_questions}
    for c in passed:
        c["questions"] = questions_map.get(c["candidate_id"], [])
    for c in human_review:
        c["questions"] = questions_map.get(c["candidate_id"], [])
    for c in top_candidates:
        c["questions"] = questions_map.get(c["candidate_id"], [])

    # Step 7 — email (still sends only top N)
