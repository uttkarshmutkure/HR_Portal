...
def process_single_candidate(candidate: dict, jd: dict, resume_cache: dict) -> dict | None:
    ...
    # 4c — Good to have (PASS / HUMAN_REVIEW only)
    good_to_have_result = {
        "clusters_total": 0, "clusters_matched": 0,
        "match_percentage": 0.0, "matched_clusters": [], "missing_clusters": []
    }
...
Q1 — skill_based      : First missing skill / weakest area vs JD.
Q2 — skill_based      : Second missing / different weak area. Must differ from Q1.
...
Return ONLY this JSON. No explanation. No markdown:
[
  {{"type": "skill_based",    "question": "question text"}},
  {{"type": "skill_based",    "question": "question text"}},
...
<style>
  ...
  .skill_based  {{ background: #fef3c7; color: #92400e; }}
...
def run_pipeline(job_id: str, top_k: int = 30, top_n: int = 5) -> dict:
    ...
    # replace the current return block with this
    return {
        "job_id"          : job_id,
        "jd_title"        : jd["title"],
